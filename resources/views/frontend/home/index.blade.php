...
@section('body')
                ...
                <div class="col-lg-3">
                    <div class="hero__categories">
                        ...
                        @foreach ($latestCategories as $category)
                            <ul>
                                <li><a
                                        href="{{ route('category.product.view', ['id' => $category->slug]) }}">{{ $category->name }}</a>
                                </li>
                            </ul>
                        @endforeach
                    </div>
                    ...
                    <div class="hero__search">
                        <div class="hero__search__form">
                            ...
                            {{-- <form id="search-form" action="{{ route('products.search') }}" method="GET">
                                <input type="text" id="search-input" name="query" placeholder="Search product"
                                    value="{{ isset($query) ? $query : '' }}">
                                <button type="submit">Search</button>
                                <button type="button" id="mic-btn" title="Voice Search" style="border: none; background: #fff; font-size: 30px;">🎤</button>
                            </form> --}}
                        </div>
                        <!-- Voice Search Button -->
                        <button type="button" id="mic-btn" title="Voice Search"
                            style="border: none; background: #fff; font-size: 30px;" data-bs-toggle="modal"
                    ...
                    @foreach ($categories as $category)
                        <div class="col-lg-3">
                            <div class="categories__item set-bg"
                                data-setbg="{{ asset('admin/category-image/' . $category->image) }}">
                                <h5><a
                                        href="{{ route('category.product.view', ['id' => $category->slug]) }}">{{ $category->name }}</a>
                                </h5>
                            </div>
                                ...
                                <ul class="product__item__pic__hover">
                                    ...
                                    @if ($product->qty > 0)
                                        <li><a href="#" data-id="{{ $product->id }}" class="directAddToCart"><i
                                                    class="fa fa-shopping-cart" style="margin-top:10px"></i></a></li>
                                    @endif
                                </ul>
                            </div>
                                ...
                                @foreach ($latestProductsDesc as $latestProduct)
                                    <a href="{{ route('product.single.view', ['id' => $latestProduct->slug]) }}"
                                        ...
                                        <div class="latest-product__item__text">
                                            ...
                                            <span>{{ $latestProduct->discount_amount ? $latestProduct->discount_amount : $latestProduct->price }}
                                ...
                                @foreach ($trendingProducts as $trendingProduct)
                                    <a href="{{ route('product.single.view', ['id' => $trendingProduct->slug]) }}"
                                        ...
                                        <div class="latest-product__item__text">
                                            ...
                                            <span>{{ $trendingProduct->discount_amount ? $trendingProduct->discount_amount : $trendingProduct->price }}
                                                {{ generalSettings('currency') }}</span>
                                        </div>
                                    </a>
                                @endforeach
                        ...
                        <div class="latest-product__slider owl-carousel">
                            <div class="latest-prdouct__slider__item">
                                @foreach ($brands as $brand)
                                    <a href="{{ route('brand.product.view', ['id' => $brand->slug]) }}"
                                        class="latest-product__item">
                                        <div class="latest-product__item__pic">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-6">
                        <div class="blog__item">
                            <div class="blog__item__pic">
                                <img src="{{ asset('admin/blog-image/' . $blog->image) }}" alt="" width="200"
                                    height="300">
                            </div>
                            <div class="blog__item__text">
                                ...
                                <h5><a
                                        href="{{ route('blog.single.view', ['id' => $blog->id]) }}">{{ $blog->title }}</a>
                                </h5>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Voice Search Modal -->
    <div class="modal fade" id="voiceSearchModal" tabindex="-1" aria-labelledby="voiceSearchModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    <input type="text" id="voiceSearchInput" class="form-control mb-3"
                        placeholder="Say product name..." readonly>
                    <p id="voiceStatus" class="text-muted">Click "Start Listening" to speak.</p>
                    <button class="btn site-btn mb-3" id="startVoice">🎙 Start Listening</button>
                    <table class="table table-bordered mt-3" id="voiceResultTable" style="display:none;">
                        <thead>
                            <tr>
                                <th>Product Name</th>
                                <th>Price</th>
                        ...
                        <tbody id="voiceResultBody"></tbody>
                    </table>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        startBtn.addEventListener('click', function() {
            ...
            recognition.onresult = function(event) {
                ...
                document.getElementById('voiceSearchInput').value = spokenText;
                document.getElementById('voiceStatus').textContent = 'Searching for "' +
                    spokenText + '" ...';
    ...
    function searchProduct(productName) {
        fetch(`/search-voice?name=${encodeURIComponent(productName)}`)
            .then(response => {
                if (!response.ok) throw new Error('Network response was not ok');
                return response.json();
            })
            .then(data => {
                const tableBody = document.getElementById('voiceResultBody');
                ...
                if (data.length === 0) {
                    ...
                    voiceStatus.innerHTML = `<span class="text-success">Found ${data.length} product(s).</span>`;
                }

                document.getElementById('voiceResultTable').style.display = 'table';
            })
            .catch(error => {
                console.error('AJAX error:', error);
                document.getElementById('voiceStatus').innerHTML =
                    '<span class="text-danger">Error fetching product info.</span>';
            });
    }
</script>
